...
def train_val_split(df, test_size=0.2):
    ...
    test_size = Size (number of rows) of returned 'test' DataFrame after split (default = 0.2)
...
def is_null(df, sum=False):
    ...
    sum(bool) = Return the sum of nulls (True), or dataframe of nulls (False), default=False
    '''
    nulls = (df != df)
    if sum is True:
        return nulls.sum()
